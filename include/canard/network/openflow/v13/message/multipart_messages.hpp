#ifndef CANARD_NET_OFP_V13_MULTIPART_MESSAGES_HPP
#define CANARD_NET_OFP_V13_MULTIPART_MESSAGES_HPP

#include <canard/network/openflow/v13/message/multipart/description.hpp>
#include <canard/network/openflow/v13/message/multipart/flow_stats.hpp>
#include <canard/network/openflow/v13/message/multipart/aggregate_stats.hpp>
#include <canard/network/openflow/v13/message/multipart/table_stats.hpp>
#include <canard/network/openflow/v13/message/multipart/port_stats.hpp>
#include <canard/network/openflow/v13/message/multipart/queue_stats.hpp>
#include <canard/network/openflow/v13/message/multipart/table_features.hpp>
#include <canard/network/openflow/v13/message/multipart/port_description.hpp>

#endif // CANARD_NET_OFP_V13_MULTIPART_MESSAGES_HPP
