...
namespace messages {
  ...
  class flow_add
    ...
    , public v13_detail::flow_entry_adaptor<flow_add, protocol::ofp_flow_mod>
  {
  public:
    ...
    flow_add(
        ...
        , std::uint32_t const buffer_id = protocol::OFP_NO_BUFFER
        , std::uint32_t const xid = get_xid())
      : flow_add{flow_entry(entry), table_id, timeouts, flags, buffer_id, xid}
    {
    }
    ...
    flow_add(
        ...
        , std::uint32_t const xid = get_xid())
      : flow_add{
          std::move(entry), table_id, v13::timeouts{0, 0}, flags, buffer_id, xid
        }
    {
    }
