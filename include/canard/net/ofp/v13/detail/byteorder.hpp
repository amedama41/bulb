...
#include <canard/net/ofp/v13/detail/fusion_adaptor.hpp>
...
namespace protocol {
  ...
  struct generic_endian_reverse_inplace
  {
    ...
    void operator()(Field& field) const noexcept
    {
    ...
    void operator()(T (&field)[N]) const noexcept
    {
      using boost::endian::endian_reverse_inplace;
    ...
    void operator()(char (&)[N]) const noexcept {}

    template <std::size_t N>
    void operator()(std::uint8_t (&)[N]) const noexcept {}
  };
