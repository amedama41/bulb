...
BOOST_AUTO_TEST_SUITE(set_mpls_ttl_test)
    ...
    BOOST_AUTO_TEST_CASE(type_definition_test)
    {
        ...
        BOOST_TEST(sut::length() == sizeof(protocol::ofp_action_mpls_ttl));
    }
    ...
    BOOST_AUTO_TEST_SUITE(equality)
      ...
      BOOST_AUTO_TEST_CASE(true_if_mpls_ttl_and_pad_are_equal)
      {
        auto const sut1 = actions::set_mpls_ttl{1};
        auto const sut2 = actions::set_mpls_ttl{1};

        BOOST_TEST((sut1 == sut2));
      }
      BOOST_AUTO_TEST_CASE(false_if_mpls_ttl_is_not_equal)
      {
        auto const sut1 = actions::set_mpls_ttl{2};
        auto const sut2 = actions::set_mpls_ttl{3};

        BOOST_TEST((sut1 != sut2));
      }
      BOOST_AUTO_TEST_CASE(false_if_pad_is_not_equal)
      {
        auto const binary = "\x00\x0f\x00\x08\x12\x00\x00\x01"_bin;
        auto it = binary.begin();
        auto const sut1 = actions::set_mpls_ttl{2};
        auto const sut2 = actions::set_mpls_ttl::decode(it, binary.end());
    ...
    BOOST_AUTO_TEST_SUITE(function_equivalent)
      ...
      {
        auto const binary = "\x00\x0f\x00\x08\x12\xab\xcd\xef"_bin;
        auto it = binary.begin();
        auto const sut1 = actions::set_mpls_ttl{0x12};
        auto const sut2 = actions::set_mpls_ttl::decode(it, binary.end());
